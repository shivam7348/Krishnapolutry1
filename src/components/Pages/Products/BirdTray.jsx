...
const BirdTrayShowcase = () => {
  ...
  const trayImages = [
    {
      src: ChickCrate,
      alt: "Chick Crate",
      description: "Durable crates with optimal ventilation for safe chick transportation",
      capacity: "50-100 chicks",
      type: "Transportation",
      features: ["Stackable design", "Easy cleaning", "Reinforced corners"]
    },
    {
      src: EggTray,
      alt: "Premium Egg Tray",
      description: "Protective egg storage solution with shock-absorbent cells",
      capacity: "30-60 eggs",
      type: "Egg Storage",
      features: ["BPA-free plastic", "Modular design", "Easy stacking"]
    },
    {
      src: ChickTransporationbox,
      alt: "Chick Transport Box",
      description: "Ventilated containers for secure poultry transportation",
      capacity: "20-50 chicks",
      type: "Transporation Box",
      features: ["Foldable design", "Removable trays", "Lightweight"]
    }
  ];
  ...
  const stats = [
    { value: "5,000+", label: "Units Sold", icon: "📦", description: "Trusted by poultry farmers" },
    { value: "99%", label: "Safety Record", icon: "🛡️", description: "Accident-free transportation" },
    { value: "10-100", label: "Capacity Range", icon: "⚖️", description: "Various size options" },
    { value: "8 Years", label: "Lifespan", icon: "⏳", description: "Durable construction" },
  ];
  ...
  const navigateImages = (direction) => {
    let newIndex;
    if (direction === 'prev') {
      newIndex = currentImageIndex === 0 ? trayImages.length - 1 : currentImageIndex - 1;
    } else {
      newIndex = currentImageIndex === trayImages.length - 1 ? 0 : currentImageIndex + 1;
    }
    setCurrentImageIndex(newIndex);
    setSelectedImage(trayImages[newIndex]);
  };
              ...
              <div className="space-y-8">
                ...
                >
                  ...
                  <h1 className="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">
                    Poultry Trays & Containers
                    <span className="block text-orange-600 mt-2 text-2xl md:text-3xl font-semibold">
                      Safe Transportation & Storage
                    </span>
                  </h1>
                  <p className="text-gray-700 leading-relaxed text-lg">
                    Our professional-grade containment solutions ensure safe handling and transportation
                    of poultry and eggs while maintaining optimal hygiene standards.
                  </p>
                  
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 p-4 rounded-lg border border-orange-100">
                    <h3 className="font-semibold text-orange-800 flex items-center">
                      <ShieldCheck className="w-5 h-5 mr-2" />
                      Quality Guaranteed
                    </h3>
                    <p className="text-gray-700 mt-1 text-sm">
                      All products come with 3-year warranty
                    </p>
                  </div>
                </motion.div>
          ...
          >
            ...
            {selectedImage.features.map((feature, i) => (
                ...
                <div className="mt-4 text-sm text-gray-300">
                  {currentImageIndex + 1} of {trayImages.length}
                </div>
              </div>
            </motion.div>
          </motion.div>
